...
from remi.application.portfolio.queries import property_ids_for_manager
...
from remi.application.core.protocols import TextSearchHit, VectorSearch  # noqa: E402

from .models import SearchHit  # noqa: E402
...
_search_log = structlog.get_logger(__name__)
    ...
    async def search(
        ...
        *,
        ...
        keyword_results = await self._vs.keyword_search(
            query, fields=_KEYWORD_FIELDS, limit=limit * 2,
        )
        ...
        if len(seen) < limit:
            ...
            try:
                semantic_results = await self._vs.semantic_search(
                    query, limit=limit, min_score=0.3, metadata_filter=metadata_filter,
                )
                ...
                _search_log.warning("search_semantic_failed", query=query[:100], exc_info=True)
