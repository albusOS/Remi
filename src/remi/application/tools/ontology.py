...
from remi.agent.graph import KnowledgeGraph, KnowledgeProvenance, ObjectTypeDef, PropertyDef
from remi.agent.signals import SignalStore
from remi.agent.types import ToolArg, ToolDefinition, ToolProvider, ToolRegistry


class KnowledgeGraphToolProvider(ToolProvider):
    def __init__(
        self,
    ...
    def register(self, registry: ToolRegistry) -> None:
        store = self._knowledge_graph

        # -- onto_signals ----------------------------------------------------------

        if self._signal_store is not None:
            _ss = self._signal_store
            ...
            registry.register(
                ...
                ToolDefinition(
                    ...
                    args=[
                        ToolArg(
                            name="signal_id",
                            description="Signal ID to explain",
                            required=True,
                        ),
                    ],
                ),
        ...
        registry.register(
            ...
            ToolDefinition(
                ...
                args=[
                    ...
                    ToolArg(
                        name="metric",
                        description="Metric: count, sum, avg, min, max",
                        required=True,
                    ),
                    ToolArg(
                        name="field",
                        description="Field to aggregate on (required for sum/avg/min/max)",
                    ),
                    ToolArg(
                        name="filters",
        ...
        registry.register(
            ...
            ToolDefinition(
                ...
                description=(
                    ...
                    "to extend the schema for discovered data."
                ),
                args=[
