...
``Note`` and ``ActionItem`` are first-class domain models and live in
the core registry.

Bootstrapping the knowledge graph is in ``application.infra.graph.seed``.
"""
...
_MODEL_REGISTRY: list[tuple[type, str] | tuple[type, str, str]] = [
    ...
    (PropertyManager, "A property manager overseeing one or more properties", "PropertyManagers"),
    # Assets
    ...
    (
        BalanceObservation,
        "A point-in-time balance snapshot capturing what a tenant owes at a moment in time",
        "BalanceObservations",
    ),
    # Operations
...
_ALL_TYPE_DEFS: list[ObjectTypeDef] = _CORE_TYPE_DEFS
...
_STRUCTURAL_LINKS: list[LinkTypeDef] = [
    ...
    LinkTypeDef(
        ...
        target_type="Property",
        cardinality="many_to_one",
        description="Entity is located at a property (lease, work order)",
    ),
    LinkTypeDef(
        name="REPORTED_BY",
        source_type="MaintenanceRequest",
        target_type="Tenant",
        cardinality="many_to_one",
        description="Work order was reported by a tenant",
    ),
    LinkTypeDef(
        name="RENEWED_FROM",
        source_type="Lease",
    ...
    LinkTypeDef(
        ...
        source_type="*",
        target_type="Document",
        description="Entity was extracted or updated from an ingested document",
    ),
]
...
_OPERATIONAL_LINKS: list[LinkTypeDef] = [
    ...
    LinkTypeDef(
        ...
        source_type="*",
        target_type="*",
        description="Causal relationship between domain phenomena",
        provenance=KnowledgeProvenance.SEEDED,
    ),
    # --- Governance bindings ---
    LinkTypeDef(
        ...
        source_type="*",
        target_type="*",
        description="Policy governs an entity type or process step",
        provenance=KnowledgeProvenance.SEEDED,
    ),
    LinkTypeDef(
        name="APPLIES_TO",
        source_type="*",
        target_type="*",
        description="Workflow step applies to an entity type",
        provenance=KnowledgeProvenance.SEEDED,
    ),
    LinkTypeDef(
        name="MANIFESTS_AS",
        source_type="*",
        target_type="*",
        description="Causal phenomenon manifests as a detectable signal",
        provenance=KnowledgeProvenance.SEEDED,
    ),
    LinkTypeDef(
        name="MEASURED_BY",
        source_type="*",
        target_type="*",
        description="Entity type is measured by a metric or signal",
        provenance=KnowledgeProvenance.SEEDED,
    ),
]
...
FK_PROJECTION_MAP: ProjectionMapping = {
    ...
    "Lease": [
        ...
        FKProjection("property_id", "AT_PROPERTY", "Property"),
    ],
    "Property": [
    ...
    "MaintenanceRequest": [
        ...
        FKProjection("property_id", "AT_PROPERTY", "Property"),
        FKProjection("tenant_id", "REPORTED_BY", "Tenant"),
