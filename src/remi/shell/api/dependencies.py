...
from remi.agent.graph import BridgedKnowledgeGraph, KnowledgeStore
...
def get_event_store(c: Container = Depends(get_container)) -> EventStore:
    return c.event_store


def get_usage_ledger(c: Container = Depends(get_container)) -> LLMUsageLedger:
    return c.usage_ledger
