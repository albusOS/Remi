...
from remi.application.intelligence.queries import DashboardResolver
from remi.application.portfolio.queries import ManagerResolver
        ...
        if self._supervisor is not None:
            ...
            async def draft_action_plan(args: dict[str, Any]) -> Any:
                ...
                spec = TaskSpec(
                    ...
                    objective=(
                        f"Analyze the manager review data for {summary.name} "
                        f"and propose prioritized action items. Focus: {focus}."
                    ),
                    input_data=review_data,
                    constraints=TaskConstraints(timeout_seconds=60),
                    metadata={
                        "source": "draft_action_plan",
                        "manager_id": manager_id,
                        "focus": focus,
                    },
                )

                result = await _supervisor.spawn_and_wait(spec, timeout=65)

                if not result.ok:
                    return {"error": result.error or "Action planner failed"}
                ...
                try:
                    ...
                    parsed = {"raw_response": result.output}
