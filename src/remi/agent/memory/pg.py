        ...
        *,
        ...
        for row in rows:
            if self._is_expired(row):
                continue
            if query_lower and query_lower not in row.key.lower() and query_lower not in row.value.lower():
                continue
            matches.append(
                MemoryEntry(
        ...
        matches.sort(key=lambda e: (e.importance, (e.created_at or datetime.min).timestamp()), reverse=True)
