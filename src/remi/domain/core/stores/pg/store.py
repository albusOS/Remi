...
from remi.agent.db.tables import (
    ActionItemRow,
    LeaseRow,
...
from remi.domain.core.stores.pg.converters import (
    ...
    async def upsert_portfolio(self, portfolio: Portfolio) -> WriteResult[Portfolio]:
        async with self._session_factory() as session:
            ...
            if existing:
                ...
                await session.commit()
                await session.refresh(existing)
                return WriteResult(entity=portfolio_from_row(existing), outcome=WriteOutcome.UPDATED)
    ...
    async def upsert_maintenance_request(self, request: MaintenanceRequest) -> WriteResult[MaintenanceRequest]:
        async with self._session_factory() as session:
            ...
            if existing:
                ...
                await session.commit()
                await session.refresh(existing)
                return WriteResult(entity=maintenance_from_row(existing), outcome=WriteOutcome.UPDATED)
    ...
    async def upsert_action_item(self, item: ActionItem) -> WriteResult[ActionItem]:
        async with self._session_factory() as session:
            ...
            if existing:
                ...
                await session.commit()
                await session.refresh(existing)
                return WriteResult(entity=action_item_from_row(existing), outcome=WriteOutcome.UPDATED)
