...
class LLMConnectionError(LLMError):
    """Transport-level connection failure to an LLM provider.

    Raised by infra/llm adapters in place of SDK-specific exceptions so
    that application-layer retry logic (retry.py) never needs to import
    vendor SDKs.
    """

    def __init__(self, message: str, *, provider: str) -> None:
        super().__init__(message, provider=provider, transient=True)
        self.code = "LLM_CONNECTION_ERROR"


class LLMTimeoutError(LLMError):
    """Request to an LLM provider timed out."""
